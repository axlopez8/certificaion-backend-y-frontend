		...
		<div class="table-wrapper">
			<div class="table-title">
				<div class="row">
					...
					<div class="col-sm-6">
						<a href="#AñadirRegistro" class="btn btn-success" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Añadir Usuario</span></a>						
					</div>
				</div>
			</div>
			<table class="table table-striped table-hover">
				...
				<tbody>
				<?php
				$json=file_get_contents("http://localhost:3000/usuarios");
			...
			?>
					<tr>
						...
						<td>
							<a href="#EditarRegistro" class="edit" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Edit" onclick='EdiForm(<?php echo $datos[$i]["idUsuario"] ?>,<?php echo $datos[$i]["idPersona"] ?>,"<?php echo $datos[$i]["usuario"]?>")'> &#xE254;</i></a>
							<a href=""  class="delete" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="Delete" onclick='eliminar(<?php echo $datos[$i]["idUsuario"] ?>,"<?php echo $datos[$i]["Nombres"] ?>")'>&#xE872;</i></a>
						</td>
					</tr>
					<?php
				}
			...
			<form action="crear.php" method="POST" >
				...
				<div class="modal-body">					
					...
					<div class="form-group">
						...
						<input type="text" id="contraseña" class="form-control" required>
					</div>
		...
		<form action="" method="POST" >
				...
				<div class="modal-body">
                    ...
                    <div id="SEL2" class="form-group">  
                            ...
                            <select id="persona2" class="form-control"  disabled>
                            <option selected >---- Nombre de las Personas ----</option>
                        ...
                        <div class="form-group">
						<label>Usuario</label>
						<input type="text" id="usuario2" class="form-control" required>
					</div>
					<div class="form-group">
						<label>Contraseña</label>
						<input type="text" id="contraseña2" class="form-control" required>
					</div>
...
function agregar(){
	...
	axios.post(`http://localhost:3000/usuarios`,params).then((res)=>{location.reload();}).catch((e)=>{console.log(e)})
}; 
...
function eliminar(id,persona){
    ...
    if (C == true) {
		axios.delete(`http://localhost:3000/usuarios/${id}`).then((res)=>{location.reload();}).catch((e)=>{console.log(e)})
	}
};

// java script para obtener valores de registros.
function EdiForm(Idu,Idp,usuario){
    console.log(Idu,Idp,usuario)
	$("#id2").val(Idu);
	$("#persona2").val(Idp);
...
function Editar(){
    ...
    params.usuario = $('#usuario2').val();
    params.contraseña = $('#contraseña2').val();

	if($('#contraseña2').val().length>0){
        axios.put(`http://localhost:3000/usuariosc/${id}`,params).then((res)=>{location.reload();}).catch((e)=>{console.log(e)})
    }else{
        axios.put(`http://localhost:3000/usuarios/${id}`,params).then((res)=>{location.reload();}).catch((e)=>{console.log(e)})
    }
